...
const queryWithTimeout = async (text, params, timeout = 5000) => {
    const timer = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Database Timeout')), timeout)
    );
    return Promise.race([pool.query(text, params), timer]);
};

export async function getServer() {
    try {
        const sql = 'SELECT nome, servidor, ativo FROM servidores WHERE ativo = true LIMIT 1';
        const result = await queryWithTimeout(sql);
        return result.rows[0];
    } catch (e) {
        console.error("Erro getServer:", e.message);
...
export async function getTheme(codigo_isp) {
    try{
        ...
        const result = await queryWithTimeout(sql, [codigo_isp]);
...
export async function getBenefit(codigo_isp, plano) {
    try {
        ...
        const result = await queryWithTimeout(sql, [codigo_isp, buscaPlano])
